    ...
    def create_tokenizer(self):
        ...
        if isinstance(self.tokenizer, tuple(MULTILINGUAL_TOKENIZERS)):
            assert data_args.target_lang is not None and data_args.source_lang is not None, (
                f"{self.tokenizer.__class__.__name__} is a multilingual tokenizer which requires --source_lang and "
                "--target_lang arguments."
            )

            self.tokenizer.src_lang = data_args.source_lang
            self.tokenizer.tgt_lang = data_args.target_lang
